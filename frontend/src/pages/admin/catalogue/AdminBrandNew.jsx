...
export default function AdminBrandNew() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const newId = created.brandId ?? created.id;

      if (logoFile && newId) {
        try {
          const formData = new FormData();
          formData.append('logo', logoFile);
          await fetch(`${API_BASE}/brands/${newId}/logo`, {
            method: 'POST',
      ...
      if (newId) {
        navigate(`/admin/catalogue/brands/${newId}/edit?created=1`);
      } else {
        navigate('/admin/catalogue/brands?created=1');
      }
    } catch {
      setSubmitError('Brand could not be saved.');
    } finally {
              ...
              <div style={styles.card}>
                ...
                <div style={styles.formGroup}>
                  ...
                  <input
                    ...
                    style={{
                      ...styles.input,
                      ...(fieldErrors.websiteUrl ? styles.inputError : {}),
                    }}
                    placeholder="https://example.com"
                    aria-describedby={fieldErrors.websiteUrl ? 'websiteUrl-error' : 'websiteUrl-hint'}
                    autoComplete="off"
                  />
                  {fieldErrors.websiteUrl ? (
                    <span id="websiteUrl-error" style={styles.fieldError} role="alert">
                      {fieldErrors.websiteUrl}
                    </span>
                  ) : (
                    <span id="websiteUrl-hint" style={styles.fieldHint}>
                      Optional. The brand's official website.
                    </span>
                  )}
                </div>
              </div>
              ...
              <div style={styles.card}>
                ...
                {!logoPreview ? (
                  <label htmlFor="logo-upload" style={styles.logoUploadArea}>
                    ...
                    <span style={{ fontSize: '12px', color: '#868e96' }}>PNG, JPG, SVG, WEBP up to 5MB</span>
                    <input
                      id="logo-upload"
                      type="file"
                      accept="image/*"
